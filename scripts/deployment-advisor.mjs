...
function buildTopologies(profile, appRanked, databaseRanked) {
  const bestFrontend = appRanked.find((x) => ['vercel', 'netlify', 'cloudflare'].includes(x.provider)) ?? appRanked[0];
  const bestApi = appRanked.find((x) => x.provider === 'render') ?? appRanked[0];
  const bestDatabase = databaseRanked[0];
  const bestSingle = appRanked[0];

  return [
    {
      mode: 'single-provider',
      score: bestSingle.score - 1,
      selection: { provider: bestSingle.provider, database: 'provider-managed-postgres' },
      reason: 'Lowest operational complexity, but runtime and database fit must both be verified.',
    },
    {
      mode: 'split',
      score: bestFrontend.score + bestApi.score + bestDatabase.score - 3,
      selection: {
        frontend: bestFrontend.provider,
        api: bestApi.provider,
        database: bestDatabase.provider,
      },
      reason: 'Best component-level fit, with additional networking, secrets, observability, egress, and rollback coordination.',
    },
  ].sort((a, b) => b.score - a.score);
}

function validate(profile) {
  const required = ['stage', 'deploymentMode', 'frontend', 'backend', 'database', 'traffic', 'monthlyBudgetUsd'];
...
try {
  ...
  console.log('Topology recommendations:');
  ...
  console.log('\nRequired release order: database/migrations → API → health check → frontend → end-to-end smoke test.');
